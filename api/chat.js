...
const PAID_CONSULTATION_STATUSES = new Set(['paid', 'ongoing', 'in_review']);

async function assertConsultationPaid(consultationId) {
  if (!consultationId) {
    throw new Error('Consultation ID wajib tersedia');
  }

  const rows = await supabaseRest(
    'GET',
    `app_consultations?id=eq.${encodeURIComponent(consultationId)}&select=id,status,app_payments(status)&limit=1`
  ).catch(() => []);
  ...
  if (!PAID_CONSULTATION_STATUSES.has(consultation.status) && !hasPaidPayment) {
    throw new Error('Chat advokat hanya tersedia setelah pembayaran konsultasi terverifikasi');
  }
  ...
  try {
    if (req.method === 'POST') {
      ...
      if (body.action === 'send-message') {
        const sessionRows = await supabaseRest(
          'GET',
          `app_chat_sessions?id=eq.${encodeURIComponent(body.chatSessionId)}&select=consultation_id&limit=1`
        ).catch(() => []);
        const consultationId = sessionRows?.[0]?.consultation_id;
        if (body.senderRole !== 'lawyer' && body.senderRole !== 'admin') {
          await assertConsultationPaid(consultationId);
        }
      ...
      if (body.senderRole !== 'lawyer' && body.senderRole !== 'admin') {
        await assertConsultationPaid(body.consultationId);
      }
