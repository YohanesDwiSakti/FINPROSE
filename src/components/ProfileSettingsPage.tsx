...
export const ProfileSettingsPage = ({ onBack }: { onBack: () => void }) => {
  ...
  const [name, setName] = useState(user?.name || 'Klien');
  const [email, setEmail] = useState(user?.email || 'client@example.com');
  ...
  const handlePhotoChange = async (file?: File) => {
    ...
    try {
      ...
      setAvatarUrl(url);
      setModal({ title: 'Foto Profil Tersimpan', description: 'Foto profil Anda berhasil diunggah dan akan tampil di akun YDA LAW OFFICE & Partners.' });
    } catch (error) {
    ...
    <div className="min-h-screen bg-brand-gray-50">
      ...
      <main className="mx-auto grid max-w-5xl grid-cols-1 gap-8 px-6 py-12 lg:grid-cols-3">
        <section className="rounded-[40px] border border-brand-gray-100 bg-brand-black p-8 text-white shadow-2xl shadow-black/20">
          ...
          <p className="mt-2 text-[10px] font-bold uppercase tracking-widest text-zinc-500">{user?.role === 'lawyer' ? 'Advokat' : user?.role === 'admin' ? 'Admin' : 'Klien'} YDA LAW OFFICE & Partners</p>
