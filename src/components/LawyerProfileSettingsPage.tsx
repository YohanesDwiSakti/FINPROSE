...
export const LawyerProfileSettingsPage = ({ onBack }: { onBack: () => void }) => {
  ...
  const [name, setName] = useState(user?.name || 'Advokat');
  ...
  const [specialty, setSpecialty] = useState(localStorage.getItem('YDA LAW OFFICE & Partners_lawyer_specialty') || 'Hukum Bisnis & Kontrak');
  const [price, setPrice] = useState(localStorage.getItem('YDA LAW OFFICE & Partners_lawyer_price') || '350000');
  const [experience, setExperience] = useState(localStorage.getItem('YDA LAW OFFICE & Partners_lawyer_experience') || '8');
  const [licenseNumber, setLicenseNumber] = useState(localStorage.getItem('YDA LAW OFFICE & Partners_lawyer_license') || 'PERADI-2026-0001');
  const [bio, setBio] = useState(localStorage.getItem('YDA LAW OFFICE & Partners_lawyer_bio') || 'Advokat terverifikasi YDA LAW OFFICE & Partners yang menangani konsultasi hukum, review dokumen, dan pendampingan perkara.');
  ...
  const saveProfile = () => {
    localStorage.setItem('YDA LAW OFFICE & Partners_user', JSON.stringify({
      id: user?.id || '',
    ...
    localStorage.setItem('YDA LAW OFFICE & Partners_lawyer_specialty', specialty);
    localStorage.setItem('YDA LAW OFFICE & Partners_lawyer_price', price);
    localStorage.setItem('YDA LAW OFFICE & Partners_lawyer_experience', experience);
    localStorage.setItem('YDA LAW OFFICE & Partners_lawyer_license', licenseNumber);
    localStorage.setItem('YDA LAW OFFICE & Partners_lawyer_bio', bio);
    setSaved(true);
