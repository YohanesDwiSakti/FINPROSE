...
import { hasAnyPaidConsultation } from '../utils/accessControl';
import { getClientDocuments } from '../services/platformData';
...
const getCaseTitle = (item: ConsultationRow) => {
  ...
  const lawyer = item.lawyer_directory?.name || 'Advokat';
...
export const DocumentVaultPage = ({ onBack }: { onBack: () => void }) => {
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      setDocs(documentRows.length ? documentRows.map(mapDocument) : getClientDocuments(user.id).map(mapDocument));
      setSelectedCaseId(current => current || consultationRows[0]?.id || null);
      setLoadError('');
    } catch (error) {
      const userDocs = getClientDocuments(user.id);
      setDocs(userDocs.map(mapDocument));
      setLoadError('');
    }
  };
  ...
  const paidCases = useMemo(
    () => cases.filter((item) => hasAnyPaidConsultation([item])),
    [cases]
  );
  const documentsLocked = paidCases.length === 0;

  if (documentsLocked) {
    return (
      <div className="min-h-screen bg-brand-gray-50 flex flex-col font-sans">
        <header className="bg-white border-b border-brand-gray-100 px-8 py-6">
          <div className="max-w-3xl mx-auto flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-xl font-bold font-display">Dokumen Konsultasi</h1>
              <p className="text-[10px] font-bold text-brand-gray-400 uppercase tracking-[0.2em] mt-0.5">Fitur premium konsultasi</p>
            </div>
          </div>
        </header>
        <main className="flex-1 flex items-center justify-center p-8">
          <div className="max-w-md rounded-[40px] border border-brand-gray-200 bg-white p-10 text-center shadow-sm">
            <Lock className="mx-auto mb-6 h-10 w-10 text-brand-gray-400" />
            <h2 className="text-2xl font-bold font-display text-brand-black">Dokumen Konsultasi Terkunci</h2>
            <p className="mt-3 text-sm leading-relaxed text-brand-gray-500">
              Unggah dan kelola dokumen kasus hanya tersedia setelah konsultasi dibayar dan diverifikasi. Rusdi AI, analisis kasus, dan pencarian advokat tetap gratis.
            </p>
            <button
              onClick={onBack}
              className="mt-8 w-full rounded-2xl bg-brand-black py-4 text-[10px] font-bold uppercase tracking-widest text-white"
            >
