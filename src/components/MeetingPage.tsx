...
export const MeetingPage = ({
  ...
  const remoteSubtitle = remoteParticipantSubtitle || (currentUserRole === 'lawyer' ? 'Klien' : lawyer.specialty);
  ...
  useEffect(() => {
    ...
    const startWebRtc = async () => {
      ...
      setCallMessage(currentUserRole === 'client' ? 'Memanggil advokat...' : 'Menunggu panggilan dari klien...');
  ...
  return (
    <div className="h-screen bg-[#0A0A0A] text-white flex flex-col overflow-hidden font-sans">
      ...
      <header className="px-6 py-4 flex items-center justify-between bg-linear-to-b from-black/50 to-transparent z-20">
        <div className="flex items-center space-x-4">
          <div className="bg-brand-black/40 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10 flex items-center space-x-3">
            ...
            <span className="text-xs font-bold uppercase tracking-widest">YDA LAW OFFICE & Partners Realtime Call</span>
          </div>
          {isRecording && (
