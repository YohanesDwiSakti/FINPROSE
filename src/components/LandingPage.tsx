...
const Navbar = ({ onAuthClick }: { onAuthClick: () => void }) => {
  ...
  const navItems = [
    { name: translate('nav.landing').toUpperCase(), id: 'home' },
    { name: translate('landing.categoriesTitle').toUpperCase(), id: 'layanan' },
    { name: translate('nav.lawyers').toUpperCase(), id: 'advokat' },
    { name: translate('nav.faq').toUpperCase(), id: 'faq' }
  ];
  ...
  return (
    <nav className="flex items-center justify-between px-8 py-6 max-w-7xl mx-auto w-full sticky top-0 bg-white/80 backdrop-blur-md z-50">
      <div className="text-2xl font-bold font-display tracking-tight cursor-pointer" onClick={() => scrollTo('home', 'HOME')}>YDA LAW OFFICE & Partners</div>
      ...
      <div className="flex items-center space-x-4">
        <div className="relative">
          <button 
            ...
            className="flex items-center space-x-1 px-3 py-2 border border-brand-gray-200 rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-brand-gray-50 transition-colors"
          >
          ...
          {langDropdownOpen && (
            <div className="absolute right-0 mt-2 w-40 bg-white border border-brand-gray-200 rounded-xl shadow-xl z-100 py-2">
              {languages.map((lang) => (
                <button
                  key={lang.code}
                  onClick={() => handleLangChange(lang.code)}
                  className={`w-full text-left px-4 py-2 text-xs font-medium hover:bg-brand-gray-50 transition-colors ${currentLang === lang.code ? 'text-brand-black font-bold' : 'text-brand-gray-400'}`}
                >
                  {lang.label}
                </button>
              ))}
        ...
        <button onClick={onAuthClick} className="btn-primary py-2 px-6 text-sm hover:scale-105 transition-transform">{translate('common.login').toUpperCase()}</button>
      </div>
    </nav>
  );
};
      ...
      <div className="grid md:grid-cols-2 gap-12 items-center">
        ...
        >
          <div className="inline-flex items-center space-x-2 px-3 py-1 bg-brand-gray-100 rounded-full">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest">{t('common.appName')}</span>
          </div>
          ...
          <div className="relative max-w-md group z-50">
            ...
            <input 
              ...
              placeholder={t('landing.categoriesSubtitle')} 
            ...
            {showDropdown && term && (
              <div className="absolute top-full mt-2 w-full bg-white border border-brand-gray-200 rounded-xl shadow-xl max-h-60 overflow-y-auto">
                {filteredLawyers.length > 0 ? filteredLawyers.map(l => (
                  ...
                  <div className="p-4 text-sm text-brand-gray-500 text-center">{t('common.loading')}</div>
                )}
              </div>
            )}
          </div>
        ...
        >
          ...
          <div className="absolute -bottom-6 -left-6 bg-white p-6 rounded-2xl shadow-xl max-w-[240px] border border-brand-gray-100">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-brand-gray-400">{t('common.status')}</span>
            </div>
            <p className="text-sm font-medium">{t('landing.topLawyersSubtitle')}</p>
          </div>
        </motion.div>
      </div>
    </section>
...
const LegalCategories = ({ onBrowse }: { onBrowse: () => void }) => {
  const { t } = useTranslation();
  const dbCategories = getCategoriesWithCounts();
  const iconMap = [Heart, Briefcase, Gavel, Scale, Users, Landmark, FileText, ShieldCheck, Star, MessageSquare];
      ...
      <div className="max-w-7xl mx-auto w-full">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 space-y-4 md:space-y-0">
          <div className="space-y-4">
            ...
            <p className="text-brand-gray-500 max-w-md">{t('landing.categoriesSubtitle')}</p>
          </div>
          <button onClick={onBrowse} className="text-sm font-bold underline hover:text-brand-gray-500 transition-colors uppercase tracking-widest">{t('common.specialty')}</button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {dbCategories.map((cat, i) => {
            ...
            >
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-brand-gray-400">{cat.lawyerCount} Advokat • {cat.consultationCount} Konsultasi</p>
            </motion.div>
            );
          })}
        </div>
...
const HowItWorks = () => {
  ...
  const steps = [
    ...
    { icon: PhoneIncoming, key: 'step3' }
  ];

  return (
    <section className="py-24 px-8 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto w-full">
        <h2 className="text-4xl font-bold font-display text-center mb-16 underline decoration-brand-gray-200 underline-offset-8">{t('howItWorks.title')}</h2>
          ...
          {steps.map((step, i) => (
            <div key={i} className="flex flex-col items-center text-center space-y-6 z-10">
              ...
              <div className="space-y-3">
                <h3 className="text-xl font-bold">{t(`howItWorks.${step.key}.title`)}</h3>
                <p className="text-brand-gray-500 text-sm leading-relaxed max-w-xs">{t(`howItWorks.${step.key}.desc`)}</p>
              </div>
            </div>
          ))}
  ...
  return (
    <section id="faq" className="bg-brand-gray-50 py-24 px-8">
      <div className="max-w-3xl mx-auto w-full">
        <h2 className="text-4xl font-bold font-display text-center mb-16">{t('landing.faqTitle')}</h2>
        <div className="space-y-4">
          {faqs.map((faq, i) => (
    ...
    <section id="advokat" className="bg-brand-black text-white py-24 px-8 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full">
        <div className="flex items-center justify-between mb-16">
          <div className="space-y-4">
            <h2 className="text-5xl font-bold font-display">{t('landing.topLawyersTitle')}</h2>
            <p className="text-zinc-500 max-w-sm">{t('landing.topLawyersSubtitle')}</p>
          </div>
          <button onClick={onBrowse} className="hidden sm:block text-sm underline text-zinc-400 hover:text-white transition-colors">{t('landing.findLawyer')}</button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            >
              <div className="mb-6 flex justify-between items-start">
                ...
                <div className="text-right">
                  ...
                  <span className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest">{lawyer.reviewCount} Reviews</span>
                </div>
              </div>
              ...
              >
                <span>{t('landing.findLawyer')}</span>
          ...
          {lawyers.length === 0 && (
            <div className="col-span-full rounded-3xl border border-white/10 bg-zinc-900/50 p-10 text-center">
              <p className="text-sm font-bold uppercase tracking-widest text-zinc-400">{t('common.loading')}</p>
            </div>
          )}
        </div>
      </div>
...
const PlatformStatsBar = () => {
  ...
  const items = [
    { label: 'Advokat Aktif', value: stats.totalLawyers },
    { label: 'Klien Terdaftar', value: stats.totalClients },
    { label: 'Konsultasi', value: stats.totalConsultations },
    { label: 'Ulasan', value: stats.totalReviews }
  ];

  return (
    <section className="border-y border-brand-gray-100 bg-white py-10 px-8">
      <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
        {items.map(item => (
          <div key={item.label} className="text-center">
            <p className="text-3xl font-bold font-display">{item.value.toLocaleString('id-ID')}</p>
            <p className="mt-2 text-[10px] font-bold uppercase tracking-widest text-brand-gray-400">{item.label}</p>
          </div>
        ))}
      </div>
    </section>
...
const LawyerShowcase = ({ title, subtitle, lawyers, onBrowse, onSelectLawyer }: {
  title: string;
  subtitle: string;
  lawyers: Lawyer[];
  onBrowse: () => void;
    ...
    <div className="max-w-7xl mx-auto">
      <div className="flex items-end justify-between mb-10">
        <div>
          ...
          <p className="mt-2 text-sm text-brand-gray-500">{subtitle}</p>
        </div>
        <button onClick={onBrowse} className="text-[10px] font-bold uppercase tracking-widest underline">Lihat Semua</button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {lawyers.slice(0, 6).map(lawyer => (
          <div key={lawyer.id} onClick={() => onSelectLawyer(lawyer)} className="border border-brand-gray-100 rounded-3xl p-6 hover:shadow-xl hover:shadow-black/5 cursor-pointer transition-all">
            <div className="flex items-center gap-4">
              <img src={lawyer.image} alt={lawyer.name} className="w-16 h-16 rounded-2xl object-cover" />
              <div>
                <p className="font-bold">{lawyer.name}</p>
                <p className="text-xs text-brand-gray-500">{lawyer.specialty}</p>
                <div className="mt-1 flex items-center gap-2 text-xs font-bold">
                  <Star className="w-3 h-3 fill-brand-black" />
                  {lawyer.rating} • Rp {lawyer.price.toLocaleString('id-ID')}
                </div>
              </div>
            </div>
          </div>
...
const RecentActivitySection = ({ onBrowse }: { onBrowse: () => void }) => {
  const reviews = getRecentReviews(6);
  const consultations = getRecentConsultations(6);

  return (
    <section className="py-20 px-8 bg-brand-gray-50">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-10">
        <div className="bg-white rounded-3xl border border-brand-gray-100 p-8">
          <h3 className="text-xl font-bold font-display mb-6">Ulasan Terbaru</h3>
          <div className="space-y-5">
            {reviews.map(review => (
              <div key={review.id} className="border-b border-brand-gray-50 pb-5 last:border-0">
                <div className="flex items-center gap-1 mb-2">
                  {[...Array(review.rating)].map((_, i) => <Star key={i} className="w-3 h-3 fill-brand-black" />)}
                </div>
                <p className="text-sm text-brand-gray-600 italic">"{review.comment}"</p>
                <p className="mt-2 text-[10px] font-bold uppercase tracking-widest text-brand-gray-400">{review.clientName} • {review.lawyerName}</p>
              </div>
            ))}
          </div>
        </div>
        <div className="bg-white rounded-3xl border border-brand-gray-100 p-8">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold font-display">Konsultasi Terbaru</h3>
            <button onClick={onBrowse} className="text-[10px] font-bold uppercase tracking-widest underline">Directory</button>
          </div>
          <div className="space-y-4">
            {consultations.map(item => (
              <div key={item.id} className="flex items-center justify-between rounded-2xl bg-brand-gray-50 p-4">
                <div>
                  <p className="text-sm font-bold">{item.issueTitle}</p>
                  <p className="text-xs text-brand-gray-500">{item.clientName} dengan {item.lawyerName}</p>
                </div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-brand-gray-400">{item.status}</span>
              </div>
            ))}
          </div>
        </div>
...
const CTASection = ({ onBrowse, onStart }: { onBrowse: () => void, onStart: () => void }) => {
  ...
  return (
    <section className="py-32 px-8 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[300px] font-bold text-brand-gray-50 z-0 select-none opacity-50">FP</div>
    ...
    <footer className="bg-brand-gray-50 py-12 px-8 border-t border-brand-gray-200">
      <div className="max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
        <div className="md:col-span-1 space-y-6">
          <div className="text-brand-black text-2xl font-bold font-display">{t('common.appName')}</div>
          <p className="text-sm text-brand-gray-400 font-medium leading-relaxed uppercase tracking-widest text-[10px]">
            {t('landing.heroSubtitle')}
          </p>
        </div>
        <div className="space-y-4">
          <h4 className="text-xs font-bold uppercase tracking-widest text-brand-black">{t('common.appName')}</h4>
          <ul className="text-sm text-brand-gray-400 space-y-2 font-medium">
            <li><a href="#" className="hover:text-brand-black">{t('nav.landing')}</a></li>
            <li><a href="#" className="hover:text-brand-black">{t('nav.lawyers')}</a></li>
          </ul>
        </div>
        <div className="space-y-4">
          <h4 className="text-xs font-bold uppercase tracking-widest text-brand-black">{t('common.help')}</h4>
          <ul className="text-sm text-brand-gray-400 space-y-2 font-medium">
            <li><a href="#" className="hover:text-brand-black">{t('common.help')}</a></li>
          </ul>
        </div>
        <div className="space-y-4">
      ...
      <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row justify-between items-center text-[10px] font-bold text-brand-gray-400 uppercase tracking-widest border-t border-brand-gray-200 pt-8">
        <p>© 2026 {t('common.appName')}. Made with Precision.</p>
...
export const LandingPage = ({ onEnterApp, onBrowseLawyers, onSelectLawyer }: { 
  ...
  return (
    <div className="min-h-screen selection:bg-brand-black selection:text-white scroll-smooth">
      ...
      <LawyerShowcase title="Featured Lawyers" subtitle="Advokat unggulan dengan rating tinggi dan respons cepat." lawyers={getFeaturedLawyers()} onBrowse={onBrowseLawyers} onSelectLawyer={onSelectLawyer} />
      <RecommendedLawyers lawyers={getTopRatedLawyers()} onBrowse={onBrowseLawyers} />
      <LawyerShowcase title="Most Consulted Lawyers" subtitle="Advokat dengan konsultasi terbanyak bulan ini." lawyers={getMostConsultedLawyers()} onBrowse={onBrowseLawyers} onSelectLawyer={onSelectLawyer} />
      <RecentActivitySection onBrowse={onBrowseLawyers} />
      <FAQSection />
