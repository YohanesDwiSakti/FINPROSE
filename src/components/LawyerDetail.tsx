...
import { getLawyerById, getReviewsForLawyer, getRelatedLawyers } from '../services/platformData';
...
export const LawyerDetail = ({ 
  ...
  const reviews = getReviewsForLawyer(lawyer.id, 4);
  const relatedLawyers = getRelatedLawyers(lawyer.id, 3);
  const profile = getLawyerById(lawyer.id);
                    ...
                    {profile && (
                      <>
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm font-medium text-brand-gray-400">Success Rate</span>
                        </div>
                        <div className="w-px h-6 bg-brand-gray-100"></div>
                        <div className="flex items-center space-x-2">
                            <span className="text-lg font-bold">{profile.consultationCount}</span>
                            <span className="text-sm font-medium text-brand-gray-400">Konsultasi</span>
                        </div>
                      </>
                    )}
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div key={review.id} className="space-y-4">
                        <div className="flex items-center space-x-1">
                            {[...Array(review.rating)].map((_, i) => <Star key={i} className="w-3 h-3 fill-brand-black text-brand-black" />)}
                        </div>
                        <p className="text-brand-gray-600 font-medium italic">"{review.comment}"</p>
                        <div className="flex items-center space-x-3">
                            <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-[10px] font-bold font-display border border-brand-gray-200">{review.clientName.split(' ').map(p => p[0]).join('').slice(0, 2)}</div>
                            <span className="text-[10px] font-bold uppercase tracking-widest text-brand-gray-400">{review.clientName} • {new Intl.DateTimeFormat('id-ID', { day: '2-digit', month: 'short' }).format(new Date(review.createdAt))}</span>
                        </div>
                    </div>
                    ))}
                </div>
            ...
            <div className="bg-brand-black text-white p-8 rounded-[40px] shadow-2xl shadow-black/20 sticky top-32 space-y-8">
                ...
                <div className="bg-zinc-900/50 p-4 rounded-2xl flex items-start space-x-3 border border-zinc-900">
                    ...
                    <p className="text-[9px] text-zinc-500 font-medium leading-relaxed uppercase tracking-wider">
                        Konsultasi dibuka di chat YDA LAW OFFICE & Partners. Telepon dan video call bisa dipakai dari dalam ruang chat.
                    </p>
                </div>
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-24 border-t border-brand-gray-100">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {relatedLawyers.map(item => (
            <div key={item.id} className="rounded-3xl border border-brand-gray-100 p-6">
              <div className="flex items-center gap-4">
                <img src={item.image} alt={item.name} className="w-14 h-14 rounded-2xl object-cover" />
                <div>
                  <p className="font-bold text-sm">{item.name}</p>
                  <p className="text-xs text-brand-gray-500">{item.specialty}</p>
                  <p className="mt-1 text-xs font-bold">★ {item.rating}</p>
                </div>
              </div>
            </div>
          ))}
