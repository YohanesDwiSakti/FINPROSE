...
import { getFilesForConsultation } from '../data/platformSeed';
...
const mapConsultation = (row: ConsultationRow): CaseView => ({
  ...
  files: getFilesForConsultation(row.id).map(file => ({
    name: file.originalName,
    date: new Intl.DateTimeFormat('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(file.createdAt)),
    ...
    fetchClientConsultations(user.id)
      ...
      .catch((error) => {
        if (mounted) {
          setCases([]);
          setSelectedCase(null);
          setLoadError(error instanceof Error ? error.message : 'Gagal memuat riwayat kasus.');
        }
      })
      .finally(() => {
        ...
        <div className="lg:col-span-8">
          {selectedCase ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="bg-white p-6 rounded-3xl border border-brand-gray-100 space-y-6">
                    ...
                    <div className="space-y-6 relative">
                        <div className="absolute left-4 top-0 bottom-0 w-[2px] bg-brand-gray-100 -ml-px"></div>
                        {[
            ...
            <div className="h-full min-h-[500px] bg-white rounded-[40px] border border-brand-gray-100 border-dashed flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-xs text-brand-gray-400 font-medium max-w-xs uppercase tracking-widest leading-relaxed">History hanya berisi kasus yang sudah selesai atau dibatalkan.</p>
            </div>
          )}
        </div>
      </main>
